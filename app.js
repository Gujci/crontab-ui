...
app.get(routes.root, async function (req, res) {
    ...
    const containers = (await docker.container.list())
        .map(it => it.data.Names.map(it => it.substr(1))[0])
        .filter(Boolean);
...
app.post(routes.save, function (req, res) {
    // new job
    if (req.body._id == -1) {
        crontab.create_new(req.body.name, req.body.command, req.body.schedule, req.body.logging, req.body.mailing, req.body.container);
    }
    // edit job
    else {
