...
crontab = function (name, command, schedule, stopped, logging, mailing, container) {
    var data = {};
    data.name = name;
    data.command = container && `docker exec -t ${container} ${command}` || command;
...
exports.create_new = function (name, command, schedule, logging, mailing, container) {
    var tab = crontab(name, command, schedule, false, logging, mailing, container);
    tab.created = new Date().valueOf();
    db.insert(tab);
};

exports.update = function (data) {
    db.update({_id: data._id}, crontab(data.name, data.command, data.schedule, null, data.logging, data.mailing, data.container));
};
...
exports.runjob = function (_id, callback) {
    db.find({_id: _id}).exec(async function (err, docs) {
        var res = docs[0];
